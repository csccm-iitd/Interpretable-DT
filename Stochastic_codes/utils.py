...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
"""
def library(xt, polyn, tensor, harmonic, exponent):
    if polyn == 0:
        polyn = 1
    ...
    if exponent == 1: 
        for i in range(len(xt)):
            for j in range(len(xt)):
                ind = ind+1
                new = np.vstack( np.exp(-xt[i,:])*xt[j,:] )
                D = np.append(D, new, axis=1)
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
"""
def sparse(xdts, ydata, polyorder, tensor, harmonic, exponent, MCMC, burn_in):
    ...
    if len(ydata) == 1: # for 1-D SDEs,
        ...
        for j in range(len(ydata[0])):
            ...
            Dtemp, nl = library(data, polyorder, tensor, harmonic, exponent)
            libr.append(Dtemp)
        libr = np.array(libr)
    ...
    noise = 0.02
    xdts = xdts + noise*np.random.normal(0, np.std(xdts), len(xdts))
